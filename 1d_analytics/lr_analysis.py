...
constrained_flow.load_state_dict(
    torch.load("../saved_models_1d/constrained_200_knots_s00005.pth")
)
...
X = X.flatten()
plt.plot(X, unconstrained_Y, c="black", label="Unconstrained (S+B)", alpha=0.5)
plt.plot(X, constrained_Y, c="tab:blue", label="Constrained (B)", alpha=0.5)
plt.plot(X, likelihood_ratio, c="tab:red", label="Likelihood ratio (S+B:B)")
...
ax2 = ax1.twinx()
ax2.plot(X, likelihood_ratio, c="tab:red", label="Likelihood ratio (S+B:B)")
